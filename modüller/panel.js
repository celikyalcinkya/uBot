...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    if (process.env.OWNER.includes(req.user.id)) {
    ...
    client.channels.get("701738747452063795").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
